...
internals.implementation = (server, options) => {
    ...
    const scheme = {
        authenticate: async (request, h) => {
            ...
            if (!authorization) {
                throw settings.unauthorized(request.auth.mode !== 'required' ? null : `${settings.accessTokenName} not found in request headers`, settings.tokenType);
            }
            ...
            if (settings.verifyJWT) {
                ...
                if (settings.keychain.length === 0) {
                    throw settings.unauthorized(`Empty Keychain: Keychain required to validate JWT`, settings.tokenType);
                }

                if (token.split('.').length !== 3) {
                    throw settings.unauthorized(`Invalid JWT: Token format not valid`, settings.tokenType);
                }
                ...
                settings.keychain.some((k) => {
                    ...
                    try {
                        ...
                        if (error.name === 'TokenExpiredError') {
                            throw settings.unauthorized(`Invalid JWT: ${error.message} expiredAt: ${error.expiredAt}`, settings.tokenType);
                        }
